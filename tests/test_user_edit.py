...
class TestUserEdit(BaseCase):
    ...
    def test_edit_just_create_user_by_unauthorized_user(self):
        ...
        for key in ('username', 'firstName', 'lastName'):
            response2 = requests.put(f"https://playground.learnqa.ru/api/user/{user_id}", data={key: new_name})
    ...
    def test_edit_just_create_user_by_another_user(self):
        ...
        # EDIT_TEST_USER
        response4 = requests.put(f"https://playground.learnqa.ru/api/user/{test_user_id}",
        ...
        # GET_TEST_USER
        response5 = requests.get(f"https://playground.learnqa.ru/api/user/{test_user_id}",
                                 headers={'x-csrf-token': editor_token},
                                 cookies={'auth_sid': editor_auth_sid}
                                 )
